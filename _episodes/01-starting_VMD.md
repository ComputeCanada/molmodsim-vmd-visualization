...
The trajectory files generated by MD simulations are large, so transferring them to your local computer can be time-consuming. By visualizing and analyzing them remotely, you could save a lot of time and efforts. That's why, we'll start our workshop by showing you how to use VND remotely. I encourage you to consider remote connection, but if you are not comfortable with this you can use your own computer.  
...
Graham has dedicated visualization nodes. You need to install TigerVNC Viewer to use them (RealVNC or any other client will not work). To start using a dedicated visualization node simply connect TigerVNC viewer to `gra-vdi.computecanada.ca`. 
